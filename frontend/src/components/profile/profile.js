...
export default function Profile({ uid }) {
  ...
  <div className="container mx-auto">
    <div className="bg-white shadow-md rounded-lg p-6 mb-8">
      <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-4">Submission History</h1>
    </div>

    <div className="flex flex-col sm:flex-row justify-between items-center mb-8 space-y-4 sm:space-y-0">
      <div className="flex overflow-x-auto space-x-4 p-4 bg-white shadow-md rounded-lg w-full sm:w-auto">
        {submissions.map((submission, index) => (
          <div
            key={index}
            onClick={() => handleSquareClick(submission)}
            className={`yearSquare cursor-pointer p-2 rounded-lg ${
              selectedSubmission?.createdAt === submission.createdAt ? 'bg-blue-500 text-white' : 'bg-gray-200'
            }`}
          >
            {submission?.createdAt?.substring(0, 4)}
          </div>
      ...
      <button
        className="bg-blue-500 text-white p-3 sm:p-4 rounded-full shadow-lg hover:bg-blue-600 transition w-full sm:w-auto"
        onClick={() => navigate('/form')}
      >
        Add Submission
      </button>
    ...
    {selectedSubmission && (
      <div className="bg-white shadow-md rounded-lg p-4 sm:p-6">
        <h2 className="text-xl sm:text-2xl font-bold mb-4">Submission Details</h2>
        <p><strong>Date:</strong> {new Date(selectedSubmission.createdAt).toLocaleDateString()}</p>
        <p><strong>Weight:</strong> {selectedSubmission.bodyInformation.weight}</p>
        <p><strong>Personal Best:</strong> {selectedSubmission.personalInformation.personalBest}</p>
        {/* Add other data fields as necessary */}
